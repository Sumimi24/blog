...
{% mermaid %}
flowchart LR
    A["Softmax Attention"] --> B["早期线性注意力"]
    B --> C["遗忘门与线性 RNN"]
    C --> D["Mamba / Mamba2 / GLA"]
    D --> E["TTT：在线学习"]
    E --> F["Delta Rule"]
    F --> G["Gated DeltaNet"]
    G --> H["反哺 Softmax Attention"]
    H --> I["PaTH / MesaNet"]
{% endmermaid %}
首先聊聊一个注意力模型中最重要的部分：
...
## TTT：将状态更新理解为在线学习
本文随即引用了更高层的Test-Time Training（即TTT）视角，它不再把 $S_t$ 仅仅看成一块被动累加的记忆，而是把它看成一个在序列内部不断被训练的模型参数。
